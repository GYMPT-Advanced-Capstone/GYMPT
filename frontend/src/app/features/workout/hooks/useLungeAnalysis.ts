...
import type { LungeAnalysisSnapshot, LungeAnalysisStatus } from "../types/lunge";

interface UseLungeAnalysisParams {
  enabled: boolean;
  goalCount: number;
  calibrationMetrics: Record<string, unknown> | null;
}

interface UseLungeAnalysisResult {
  analysis: LungeAnalysisSnapshot;
...
const DISCONNECTED_STATUS: LungeAnalysisStatus = "insufficient_visibility";
const DISCONNECTED_MESSAGE = "백엔드 연결이 끊겼습니다. 다시 연결을 시도해주세요.";

const STATUS_MAP: Record<LungeAnalysisStatus, true> = {
  idle: true,
  tracking: true,
  insufficient_visibility: true,
};

const INITIAL_ANALYSIS: LungeAnalysisSnapshot = {
  timestampMs: 0,
  status: "idle",
  feedbackMessage: "",
  fullRepCount: 0,
...
function createInitialAnalysis(): LungeAnalysisSnapshot {
...
function isLungeAnalysisStatus(value: unknown): value is LungeAnalysisStatus {
...
export function useLungeAnalysis({
  enabled,
  goalCount,
  calibrationMetrics,
}: UseLungeAnalysisParams): UseLungeAnalysisResult {
  const [analysis, setAnalysis] = useState<LungeAnalysisSnapshot>(createInitialAnalysis);
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      setAnalysis((prev) => {
        ...
        const nextStatus = isLungeAnalysisStatus(feedback.status) ? feedback.status : prev.status;
