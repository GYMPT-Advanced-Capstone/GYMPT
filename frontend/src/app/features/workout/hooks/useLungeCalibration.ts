...
import { buildLungeObservation } from "../utils/lunge";
...
export type LungeCalibrationStep =
  | "idle"
  | "top_waiting"
  | "top_counting"
...
interface UseLungeCalibrationParams {
  enabled: boolean;
  exerciseId: number;
  onSuccess: () => void;
}

interface UseLungeCalibrationResult {
  step: LungeCalibrationStep;
  phase: "top" | "bottom";
...
export function useLungeCalibration({
  enabled,
  exerciseId,
  onSuccess,
}: UseLungeCalibrationParams): UseLungeCalibrationResult {
  const [step, setStep] = useState<LungeCalibrationStep>("idle");
  ...
  const lastSpokenStepRef = useRef<LungeCalibrationStep | null>(null);
  ...
  useEffect(() => {
    ...
    const messages: Partial<Record<LungeCalibrationStep, string>> = {
      top_waiting: "측면으로 서서 런지 시작 자세를 유지해주세요.",
      top_counting: "좋습니다. 런지 시작 자세 측정을 시작합니다. 2초만 유지해주세요.",
      transition_to_bottom: "좋습니다. 이제 런지로 내려간 자세를 측정합니다. 천천히 내려가 유지해주세요.",
      bottom_waiting: "런지로 내려간 자세를 유지해주세요. 안정되면 자동으로 2초간 측정합니다.",
      bottom_counting: "런지로 내려간 자세 측정을 시작합니다. 2초만 유지해주세요.",
      saving: "런지 기준 범위를 저장하고 있습니다.",
      complete: "런지 기준 범위 설정이 완료되었습니다. 운동을 시작합니다.",
    };
  ...
  const onPoseLandmarks = useCallback((landmarks: NormalizedLandmark[] | null, timestampMs: number) => {
    ...
    const observation = buildLungeObservation(landmarks);
    if (!observation) {
      captureStartedAtRef.current = null;
    ...
    if (topSamples.length === 0 || bottomSamples.length === 0) {
      setCalibrationError("시작 자세와 내려간 자세를 모두 측정해주세요.");
      return;
    }
    ...
    void workoutApi.createCalibration({
      ...
      samplesRef.current.bottom = [];
      setStep("bottom_waiting");
      setCalibrationError(error instanceof Error ? error.message : "런지 기준 범위 저장에 실패했습니다.");
    }).finally(() => {
      setIsSavingCalibration(false);
    });
  ...
  if (step === "top_waiting") {
    noticeMessage = "측면으로 서서 런지 시작 자세를 유지해주세요. 안정되면 자동으로 2초간 측정합니다.";
  } else if (step === "top_counting") {
    noticeMessage = "런지 시작 자세 측정 중입니다. 2초만 그대로 유지해주세요.";
  } else if (step === "transition_to_bottom") {
    noticeMessage = "좋습니다. 이제 런지로 내려간 자세를 측정합니다. 천천히 내려가 유지해주세요.";
  } else if (step === "bottom_waiting") {
    noticeMessage = "런지로 내려간 자세를 유지해주세요. 안정되면 자동으로 2초간 측정합니다.";
  } else if (step === "bottom_counting") {
    noticeMessage = "런지로 내려간 자세 측정 중입니다. 2초만 그대로 유지해주세요.";
  } else if (step === "saving") {
    noticeMessage = "런지 기준 범위를 저장하고 있습니다.";
  }
