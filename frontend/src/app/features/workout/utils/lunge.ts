...
export interface LungeTrackedLandmark {
  x: number;
  y: number;
  visibility?: number;
}

export interface LungeTrackedLandmarks {
  side: "left" | "right";
  shoulder: LungeTrackedLandmark;
  hip: LungeTrackedLandmark;
  knee: LungeTrackedLandmark;
  ankle: LungeTrackedLandmark;
  foot: LungeTrackedLandmark;
}

export interface LungeMetrics extends Record<string, number> {
  kneeAngle: number;
  hipAngle: number;
...
export interface LungeObservation {
  trackedLandmarks: LungeTrackedLandmarks;
...
function distance(a: LungeTrackedLandmark, b: LungeTrackedLandmark) {
  return Math.hypot(a.x - b.x, a.y - b.y);
}

function calculateAngle(a: LungeTrackedLandmark, b: LungeTrackedLandmark, c: LungeTrackedLandmark) {
...
function calculateTorsoLeanAngle(shoulder: LungeTrackedLandmark, hip: LungeTrackedLandmark) {
...
function sideVisibilityScore(sideLandmarks: LungeTrackedLandmarks | null) {
  if (!sideLandmarks) {
    return -1;
  }
  return [
...
export function buildLungeObservation(
  landmarks: NormalizedLandmark[] | null,
): LungeObservation | null {
