import { Heart, Image as ImageIcon, MessageCircle, Plus, Trash2, User, Send, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export function Community() {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      const serverPosts = Array.isArray(response.data)
        ? response.data
        : [];
  ...
  const handleLikeUpdate = async (board_no: number) => {
    setPosts(prev => prev.map(p => {
      if (p.board_no === board_no) {
        return { 
          ...p, 
          is_liked: !p.is_liked, 
          likes: p.is_liked ? Math.max(0, p.likes - 1) : p.likes + 1 
        };
      }
      return p;
    }));
    try {
      await axios.post(`${API_BASE_URL}/api/v1/board/${board_no}/likes`, {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem("gympt_access_token")}` }
      });
  ...
  return (
    <div className="flex justify-center items-start w-full" style={{ minHeight: '100dvh', backgroundColor: '#111111' }}>
      <div className="flex flex-col relative overflow-y-auto scrollbar-hide" style={{ width: '100%', maxWidth: '390px', minHeight: '100dvh', backgroundColor: '#1A1A1A', paddingBottom: 88 }}>
          ...
          {posts.length === 0 ? (
            ...
            posts.map((post, index) => (
              <PostCard
                ...
                onDelete={(no: number) =>
                  setPosts((prev) =>
                    prev.filter((p) => p.board_no !== no)
                  )
                }
                onLikeUpdate={handleLikeUpdate}
                onOpenComments={() =>
                  setActiveCommentPostId(post.board_no)
                }
        ...
        <AnimatePresence>
          {activeCommentPostId && (
            <CommentsBottomSheet 
              ...
              onCountUpdate={(id: number, type: 'plus'|'minus') => setPosts(prev => prev.map(p => p.board_no === id ? { ...p, comments_count: type === 'plus' ? p.comments_count + 1 : p.comments_count - 1 } : p))}
              onApiError={handleApiError}
            />
          )}
        </AnimatePresence>
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(`/edit-post/${post.board_no}`)}
            ...
            onClick={async () => {
              ...
              try {
                await axios.delete(
                  ...
                  {
                    headers: {
                      Authorization: `Bearer ${localStorage.getItem("gympt_access_token")}`
                    }
                  }
                );
      ...
      <div className="p-5 pt-2 flex flex-col gap-3">
        <div className="flex items-center gap-4">
          <button
            onClick={() => onLikeUpdate(post.board_no)}
...
function CommentsBottomSheet({ onClose, postId, onCountUpdate, onApiError }: any) {
  ...
  const fetchComments = useCallback(async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/v1/board/${postId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("gympt_access_token")}` }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await axios.patch(`${API_BASE_URL}/api/v1/board/comments/${editId}`, { content: commentInput }, { headers: { Authorization: `Bearer ${token}` } });
        setComments(prev => prev.map(c => c.comment_no === editId ? { ...c, content: commentInput } : c));
        setEditId(null);
      } else {
        const res = await axios.post(`${API_BASE_URL}/api/v1/board/${postId}/comments`, { content: commentInput }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/60 z-[100] backdrop-blur-sm max-w-[390px] mx-auto" />
      <motion.div 
        initial={{ y: "100%" }} 
        animate={{ y: 0 }} 
        exit={{ y: "100%" }} 
      ...
      >
        <div className="flex flex-col items-center pt-3 pb-2" onClick={onClose}>
          ...
          <h2 className="text-[16px] font-bold text-white mb-2">댓글</h2>
        </div>
          ...
          {comments.length > 0 ? comments.map((c) => (
            <div key={c.comment_no} className="flex gap-3">
              ...
              <div className="flex-1">
                <span className="text-[13px] font-bold text-white block mb-1">{c.writer}</span>
                <p className="text-[14px] text-white/80 leading-relaxed">{c.content}</p>
                <div className="flex gap-4 mt-2">
                  <button onClick={() => { setEditId(c.comment_no); setCommentInput(c.content); }} className="text-[11px] text-[#3FFDD4]/70">수정</button>
                  <button onClick={async () => {
                    if (!confirm("댓글을 삭제하시겠습니까?")) return;
                    try {
                      await axios.delete(`${API_BASE_URL}/api/v1/board/comments/${c.comment_no}`, { headers: { Authorization: `Bearer ${localStorage.getItem("gympt_access_token")}` } });
                      setComments(prev => prev.filter(item => item.comment_no !== c.comment_no));
                      onCountUpdate(postId, 'minus');
                    } catch (e) { onApiError(e); }
                  }} className="text-[11px] text-white/20">삭제</button>
                </div>
              </div>
            </div>
          )) : (
        ...
        <div className="p-4 bg-[#1A1A1A] border-t border-white/10 pb-10">
          <form onSubmit={handleSubmit} className="flex items-center gap-3 bg-white/5 rounded-full px-5 py-3 border border-white/10">
            <input 
              type="text" 
              value={commentInput} 
              onChange={(e) => setCommentInput(e.target.value)} 
              placeholder={editId ? "내용 수정 중..." : "댓글 달기..."} 
              className="flex-1 bg-transparent text-white outline-none text-[14px]" 
            />
            <button type="submit" disabled={!commentInput.trim()} className={commentInput.trim() ? "text-[#3FFDD4]" : "text-white/10"}>
              <Send size={20} />
            </button>
          </form>
        </div>
