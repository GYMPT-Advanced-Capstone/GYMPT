...
const DUMMY_IMAGES = {
  ...
  workout_gear: "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=1000&auto=format&fit=crop"
};
...
export function Community() {
  ...
  const handleCommentCountUpdate = (board_no: number, type: 'plus' | 'minus') => {
    setPosts(prev => prev.map(p => 
      p.board_no === board_no ? { ...p, comments_count: type === 'plus' ? p.comments_count + 1 : Math.max(0, p.comments_count - 1) } : p
    ));
  };

  return (
    <div className="flex justify-center items-start w-full" style={{ minHeight: '100dvh', backgroundColor: '#111111' }}>
      <div className="flex flex-col relative overflow-y-auto" style={{ width: '100%', maxWidth: '390px', minHeight: '100dvh', backgroundColor: '#1A1A1A', paddingBottom: 88 }}>
        <header className="sticky top-0 z-40 bg-[#1A1A1A]/80 backdrop-blur-xl border-b border-white/5 pt-12 pb-4 px-6 flex justify-between items-center">
          ...
          <button onClick={() => navigate('/create-post')} className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white active:scale-95 transition-transform">
        ...
        <div className="flex-1 p-5 flex flex-col gap-6">
          {posts.map((post, index) => (
            <PostCard 
              ...
              post={post} 
              index={index} 
              onDelete={(no: number) => setPosts(prev => prev.filter(p => p.board_no !== no))}
    ...
    <>
      ...
      >
              ...
              <div className="flex gap-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-3 mt-1">
                    <button onClick={() => { setEditingCommentId(comment.comment_no); setCommentInput(comment.content); }} className="text-[11px] text-[#3FFDD4]/60 hover:text-[#3FFDD4]">수정</button>
                    <button onClick={async () => {
                      if (!window.confirm("댓글을 삭제할까요?")) return;
                      try {
                        const token = localStorage.getItem("gympt_access_token");
                        await axios.delete(`http://localhost:8000/api/v1/board/comments/${comment.comment_no}`, { headers: { Authorization: `Bearer ${token}` } });
                        setComments(prev => prev.filter(c => c.comment_no !== comment.comment_no));
                        onCountUpdate(postId, 'minus');
                      } catch (error) { onApiError(error); }
                    }} className="text-[11px] text-white/20 hover:text-red-400">삭제</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="p-4 bg-[#1A1A1A] border-t border-white/10 pb-10">
          <form onSubmit={handleSubmit} className="flex items-center gap-3 bg-white/5 rounded-full px-4 py-2.5 border border-white/10 focus-within:border-[#3FFDD4]/30">
            <input 
