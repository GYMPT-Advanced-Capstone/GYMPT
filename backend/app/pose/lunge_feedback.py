...
LUNGE_VIEW_MESSAGE = "몸의 측면 관절이 보이도록 카메라에 몸을 맞춰주세요."
LUNGE_GOOD_MESSAGE = "좋은 런지 자세예요."
LUNGE_DEPTH_MESSAGE = "런지 깊이가 조금 부족해요. 앞무릎을 조금 더 굽혀주세요."
LUNGE_TOO_DEEP_MESSAGE = "런지 깊이가 너무 깊어요."
LUNGE_TORSO_LEAN_MESSAGE = "상체가 너무 숙여졌어요. 가슴을 세워주세요."
LUNGE_KNEE_TRACK_MESSAGE = "앞무릎이 발끝 방향과 같은 방향을 향하게 해주세요."
LUNGE_BOTTOM_GOOD_DEPTH = 0.55
LUNGE_GOOD_DEPTH_THRESHOLD = 1.0
LUNGE_IDEAL_TOP_KNEE_ANGLE = 170.0
LUNGE_IDEAL_TOP_HIP_ANGLE = 165.0
LUNGE_IDEAL_BOTTOM_KNEE_ANGLE = 90.0
LUNGE_IDEAL_BOTTOM_HIP_ANGLE = 95.0
LUNGE_TOO_DEEP_ANGLE_TOLERANCE = 35.0
LUNGE_TOP_ZONE_MAX = 0.30
LUNGE_MIN_REP_DURATION_MS = 250.0
LUNGE_TORSO_LEAN_DEFAULT_THRESHOLD = 55.0
LUNGE_TORSO_LEAN_TOLERANCE = 30.0


class LungeFeedbackProcessor:
        ...
        *,
        ...
        if observation is None:
            return {
                ...
                "feedbackMessage": LUNGE_VIEW_MESSAGE,
        ...
        feedback_message = warning_message or LUNGE_GOOD_MESSAGE
        ...
        if zone == "top" and state.movement_zone != "top" and state.rep_active:
            ...
            if rep_duration_ms >= LUNGE_MIN_REP_DURATION_MS:
                rep_completed = True
    ...
    def _resolve_observation(
        ...
        bottom_knee_for_depth = self._resolve_bottom_knee_angle(bottom_knee)
        ...
        bottom_hip_for_depth = self._resolve_bottom_hip_angle(bottom_hip)

        knee_progress = self._progress_between(
            start=self._resolve_top_knee_angle(top_knee),
        ...
        hip_progress = self._progress_between(
            start=self._resolve_top_hip_angle(top_hip),
            end=bottom_hip_for_depth,
            current=hip_angle,
            invert=True,
        )
    ...
    def _resolve_zone(depth_progress: float) -> str:
        if depth_progress >= LUNGE_BOTTOM_GOOD_DEPTH:
            return "bottom"
        if depth_progress <= LUNGE_TOP_ZONE_MAX:
            return "top"
        return "mid"
    ...
    def _resolve_warning(
        ...
        if target_torso is None:
            torso_threshold = LUNGE_TORSO_LEAN_DEFAULT_THRESHOLD
        else:
            torso_threshold = max(
                LUNGE_TORSO_LEAN_DEFAULT_THRESHOLD,
                target_torso + LUNGE_TORSO_LEAN_TOLERANCE,
            )
        ...
        if observation["torsoLeanAngle"] > torso_threshold:
            return "torso_lean", LUNGE_TORSO_LEAN_MESSAGE
        if observation["kneeAnkleOffsetX"] > knee_offset_threshold:
            return "knee_track", LUNGE_KNEE_TRACK_MESSAGE
        return None, None
    ...
    def _resolve_depth_warning(
        ...
        *,
        ...
        knee_too_deep = bottom_knee_for_depth is not None and (
            observation["kneeAngle"]
            < bottom_knee_for_depth - LUNGE_TOO_DEEP_ANGLE_TOLERANCE
        )
        hip_too_deep = bottom_hip_for_depth is not None and (
            observation["hipAngle"]
            < bottom_hip_for_depth - LUNGE_TOO_DEEP_ANGLE_TOLERANCE
        )

        if bottom_knee_for_depth is not None and bottom_hip_for_depth is not None:
            is_too_deep = knee_too_deep and hip_too_deep
        else:
            is_too_deep = knee_too_deep or hip_too_deep

        if is_too_deep:
            return "depth_high", LUNGE_TOO_DEEP_MESSAGE

        depth_progress = observation.get("depthProgress")
        if (
            require_depth
            and depth_progress is not None
            and depth_progress < LUNGE_GOOD_DEPTH_THRESHOLD
        ):
            return "depth_low", LUNGE_DEPTH_MESSAGE

        if (
            (bottom_knee_for_depth is not None or bottom_hip_for_depth is not None)
            and depth_progress is not None
            and depth_progress >= LUNGE_BOTTOM_GOOD_DEPTH
            and depth_progress < LUNGE_GOOD_DEPTH_THRESHOLD
        ):
            return "depth_low", LUNGE_DEPTH_MESSAGE
    ...
    @staticmethod
    def _resolve_bottom_knee_angle(value: float | None) -> float | None:
        if value is None:
            return LUNGE_IDEAL_BOTTOM_KNEE_ANGLE
        return max(LUNGE_IDEAL_BOTTOM_KNEE_ANGLE, value)

    @staticmethod
    def _resolve_bottom_hip_angle(value: float | None) -> float | None:
        if value is None:
            return LUNGE_IDEAL_BOTTOM_HIP_ANGLE
        return max(LUNGE_IDEAL_BOTTOM_HIP_ANGLE, value)

    @staticmethod
    def _resolve_top_knee_angle(value: float | None) -> float:
        if value is None:
            return LUNGE_IDEAL_TOP_KNEE_ANGLE
        return value

    @staticmethod
    def _resolve_top_hip_angle(value: float | None) -> float:
        if value is None:
            return LUNGE_IDEAL_TOP_HIP_ANGLE
        return value
    ...
    def _resolve_representative_feedback(self, state: Any) -> tuple[str, str]:
        ...
        if state.current_rep_max_depth < LUNGE_GOOD_DEPTH_THRESHOLD:
            return "depth_low", LUNGE_DEPTH_MESSAGE
        warning_counts.pop("depth_low", None)

        if not warning_counts:
            return "good", LUNGE_GOOD_MESSAGE
        ...
        message_map = {
            "depth_high": LUNGE_TOO_DEEP_MESSAGE,
            "depth_low": LUNGE_DEPTH_MESSAGE,
            "torso_lean": LUNGE_TORSO_LEAN_MESSAGE,
            "knee_track": LUNGE_KNEE_TRACK_MESSAGE,
            "good": LUNGE_GOOD_MESSAGE,
        }
