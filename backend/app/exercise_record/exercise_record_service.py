...
from app.core.openai_client import generate_workout_feedback
...
class ExerciseRecordService:
    ...
    def create(
        ...
        if data.analysis is not None:
            rep_feedback_codes = [
                rep.representative_feedback_code for rep in data.analysis.reps
            ]
            ai_feedback = generate_workout_feedback(
                exercise_type=data.analysis.exercise_type or "unknown",
                total_reps=data.count,
                duration_seconds=data.duration,
                rep_feedback_codes=rep_feedback_codes,
            )
            if ai_feedback:
                self.repo.update_ai_feedback(created_record, ai_feedback)
